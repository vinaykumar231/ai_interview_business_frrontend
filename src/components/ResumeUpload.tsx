...
const ResumeDropzone: React.FC<ResumeDropzoneProps> = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={`p-3 mx-auto w-16 h-16 rounded-full mb-4 ${
          isEnabled ? "bg-blue-500/20" : "bg-gray-800"
        }`}
      >
        <Upload
          className={`w-10 h-10 ${
...
const ResumeUpload: React.FC = () => {
  ...
  return (
    <>
      <div className="m-9 min-h-screen ">
        {/* <h2 className="text-xl font-semibold mb-4">Upload Resumes</h2> */}
        <div className="mr-auto p-4 flex justify-between ">
          <form onSubmit={handleUpload} className="space-y-4 w-[42%]">
            <div>
              <h2 className="text-xl font-semibold mb-4 text-center">
                Upload Resumes
              </h2>
              <label className="block text-[16px] font-medium">Job Title</label>
              <input
                type="text"
                value={jobTitle}
                onChange={handleJobTitleChange}
                className="mt-1 p-[20px] border rounded w-full"
            ...
            <div>
              ...
              <textarea
                ...
                className="mt-1 p-[55px] pl-[20px] pt-[20px] border rounded w-full"
            ...
            {uploadSuccess && (
              <div className="text-green-500 mt-2">Upload successful!</div>
            )}
          </form>
          ...
          {results.length > 0 && (
            <div className="mt-8 w-[50%] ">
              <div className="mt-8 w-[50%] text-end ml-[52%]">
                <a href="/ai_hr/selected_candidate">
                  <button className="mt-2 mb-2 px-4 py-2 bg-white text-blue-500 border border-blue-500 rounded-md text-center">
                    Go to Selected Candidates
                  </button>
                </a>
              </div>
              <h3 className="text-lg font-semibold text-center">
                Resume Selection Results
              </h3>
              {results.map((result, index) => (
                <div
                  key={index}
                  className="mt-4 border p-4 rounded bg-[#7385B7]"
                >
                  <button
                  ...
                  {active === index &&
                    result.resume_selection_result?.candidate_info && (
                      <div>
                        {/* <p>Name: {result.resume_selection_result.candidate_info.name}</p> */}
                        <p>
                          Email:{" "}
                          {result.resume_selection_result.candidate_info.email}
                        </p>
                        <p>
                          Phone:{" "}
                          {result.resume_selection_result.candidate_info.phone}
                        </p>
                        <p>
                          overall_score:{" "}
                          {result?.resume_selection_result?.overall_score}
                        </p>
                        <p>
                          relevance_score:{" "}
                          {result?.resume_selection_result?.relevance_score}
                        </p>
                        <p>
                          cultural_fit_score:{" "}
                          {result?.resume_selection_result?.cultural_fit_score}
                        </p>
                        <p>
                          skills_fit_score:{" "}
                          {result?.resume_selection_result?.skills_fit_score}
                        </p>
                        <p>
                          experience_match_score:{" "}
                          {
                            result?.resume_selection_result
                              ?.experience_match_score
                          }
                        </p>
                      </div>
                    ...
                    result.resume_selection_result?.candidate_info && (
                      <div>
                        ...
                        <p>
                          Qustion1:{" "}
                          {
                            result?.resume_selection_result?.questions
                              ?.generated_questions?.all_questions?.Qustion1
                          }
                        </p>
                        <p>
                          Qustion2:{" "}
                          {
                            result?.resume_selection_result?.questions
                              ?.generated_questions?.all_questions?.Qustion2
                          }
                        </p>
                        <p>
                          Qustion3:{" "}
                          {
                            result?.resume_selection_result?.questions
                              ?.generated_questions?.all_questions?.Qustion3
                          }
                        </p>
                        <p>
                          Qustion4:{" "}
                          {
                            result?.resume_selection_result?.questions
                              ?.generated_questions?.all_questions?.Qustion4
                          }
                        </p>
                        <p>
                          Qustion5:{" "}
                          {
                            result?.resume_selection_result?.questions
                              ?.generated_questions?.all_questions?.Qustion5
                          }
                        </p>
                      </div>
                    )}
                  ...
                  {active === index &&
                    result.resume_selection_result?.strengths &&
                    result.resume_selection_result.strengths.length > 0 && (
                      <div>
                        <h5 className="font-bold">Strengths:</h5>
                        <ul>
                          {result.resume_selection_result.strengths.map(
                            (strength, idx) => (
                  ...
                  {active === index &&
                    result.resume_selection_result?.weaknesses &&
                    result.resume_selection_result.weaknesses.length > 0 && (
                      <div>
                        <h5 className="font-bold">Weaknesses:</h5>
                        <ul>
                          {result.resume_selection_result.weaknesses.map(
                            (weakness, idx) => (
                  ...
                  {active === index &&
                    result.resume_selection_result?.recommendations &&
                    result.resume_selection_result.recommendations.length >
                      0 && (
                      <div>
                        <h5 className="font-bold">Recommendations:</h5>
                        <ul>
                          {result.resume_selection_result.recommendations.map(
                            (recommendation, idx) => (
                  ...
                  {active === index &&
                    result.resume_selection_result?.missing_elements &&
                    result.resume_selection_result.missing_elements.length >
                      0 && (
                      <div>
                        <h5 className="font-bold">Missing Elements:</h5>
                        <ul>
                          {result.resume_selection_result.missing_elements.map(
                            (missing, idx) => (
